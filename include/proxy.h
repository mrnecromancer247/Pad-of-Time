...
// Global hotkey (see [MenuMode] in PadOfTime.ini): call once per real
// GetDeviceState() poll, unconditionally, in every mode. Sends a real
// Escape keypress when the configured button is pressed/released, so
// players can pause via the button the game itself doesn't respond to over
// DirectInput.
void Proxy_HandleGlobalHotkeys();

// Menu-mouse mode (see [MenuMode] in PadOfTime.ini): call once per real
// GetDeviceState() poll. Handles the toggle button, moves the real Windows
// cursor from the right stick and fires left-clicks from a button while
// active. Returns true if menu-mouse mode is active THIS call - the caller
// should then leave the DirectInput buffer neutral (zeroed) instead of
// filling it with real pad state, so the game doesn't also react to the
// toggle/click buttons as gameplay input.
